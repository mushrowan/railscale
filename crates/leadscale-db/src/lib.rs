...
use leadscale_types::{Config, Node, NodeId, PreAuthKey, User, UserId};

/// result type for database operations.
pub type Result<T> = std::result::Result<T, Error>;

/// database trait for leadscale storage operations.
///
...
mod tests {
    ...
    async fn setup_test_db() -> LeadscaleDb {
        LeadscaleDb::new_in_memory().await.unwrap()
    }
    ...
    async fn test_node_crud() {
        use leadscale_types::{DiscoKey, MachineKey, NodeKey, RegisterMethod};
