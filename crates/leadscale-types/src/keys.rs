...
//!
//! wrap the raw key bytes and provide serialization support
//! actual cryptographic operations will be implemented in leadscale-proto

use serde::{Deserialize, Serialize};

/// machine key - identifies a physical device
///
/// stable across node key rotations and is used
/// for machine-level authentication
#[derive(Debug, Clone, PartialEq, Eq, Hash, Serialize, Deserialize)]
pub struct MachineKey(Vec<u8>);

impl MachineKey {
    /// create new machine key from raw bytes
...
///
/// can be rotated and is used for the noise protocol handshake
#[derive(Debug, Clone, PartialEq, Eq, Hash, Serialize, Deserialize)]
pub struct NodeKey(Vec<u8>);

impl NodeKey {
    /// create new node key from raw bytes
    pub fn from_bytes(bytes: Vec<u8>) -> Self {
        Self(bytes)
    }
...
impl DiscoKey {
    /// create new disco key from raw bytes
    pub fn from_bytes(bytes: Vec<u8>) -> Self {
        Self(bytes)
    }
