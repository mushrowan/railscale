...
use leadscale_types::{Node, NodeId, NodeView};

/// in-memory cache for nodes with copy-on-write semantics
///
/// rust equivalent of go's nodestore in headscale
/// it provides:
/// - fast reads without locking (using arc)
/// - copy-on-write updates
/// - thread-safe concurrent access
pub struct NodeStore {
    /// actual node data, wrapped in arc for copy-on-write
...
impl NodeStore {
    /// create new nodestore with initial nodes
    ...
    ///
    /// uses copy-on-write: creates new hashmap with the updated
    ...
    pub fn set_online(&self, id: NodeId, online: bool) {
        ...
        // special case that doesn't need full cow
        let nodes = self.nodes.read().unwrap();
        if let Some(view) = nodes.get(&id) {
            // note: limitation - we can't easily update
    ...
    ///
    /// called after node updates to recalculate which nodes
...
mod tests {
    use super::*;
    use chrono::Utc;
    use leadscale_types::{DiscoKey, MachineKey, NodeKey, RegisterMethod, UserId};
